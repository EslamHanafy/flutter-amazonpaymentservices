...
public class SwiftFlutterAmazonpaymentservicesPlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    if call.method == "normalPay" {
      guard let param = call.arguments as? [String: Any] else {
        print("AmazonPaymentService: Failed to cast arguments to [String: Any]")
        result(
          FlutterError(
            code: "INVALID_ARGS", message: "Could not parse arguments", details: call.arguments))
        return
      }
      print("AmazonPaymentService: param = \(param)")

      guard let requestParam = param["requestParam"] as? [String: String] else {
        print("AmazonPaymentService: Failed to cast requestParam to [String: String]")
        print("AmazonPaymentService: requestParam type: \(type(of: param["requestParam"]))")
        result(
          FlutterError(
            code: "INVALID_REQUEST", message: "requestParam must be [String: String]",
            details: param["requestParam"]))
        return
      }
  ...
  private func handleApplePay(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    guard let args = call.arguments as? [String: Any],
      let requestParam = args["requestParam"] as? [String: Any]
    else {
      result(
        FlutterError(
          code: "INVALID_PARAMETERS", message: "Missing Apple Pay parameters",
    ...
    guard let amount = requestParam["displayAmount"] as? String,
      ...
      let merchantId = requestParam["merchantIdentifier"] as? String
    else {
      result(
        FlutterError(
          code: "INVALID_PARAMETERS", message: "Missing required Apple Pay parameters",
    ...
    if let transactionDetails = requestParam["transactionDetails"] as? [String: String] {
      if let digitalWallet = transactionDetails["digital_wallet"], digitalWallet == "APPLE_PAY" {
        ...
        result(
          FlutterError(
            code: "MISSING_TRANSACTION_DETAILS",
            message: "Missing/Invalid parameter digital_wallet: APPLE_PAY expected", details: nil))
        return
      }
    } else {
      result(
        FlutterError(
          code: "MISSING_TRANSACTION_DETAILS", message: "Transaction details missing", details: nil)
      )
      return
    }
    ...
    guard let viewController = UIApplication.shared.windows.first?.rootViewController else {
      result(
        FlutterError(
          code: "NO_VIEW_CONTROLLER", message: "No view controller available", details: nil))
      return
    }
    ...
    if let paymentController = PKPaymentAuthorizationViewController(paymentRequest: paymentRequest)
    {
      ...
      result(
        FlutterError(
          code: "APPLE_PAY_UNAVAILABLE", message: "Unable to present Apple Pay", details: nil))
    }
...
extension SwiftFlutterAmazonpaymentservicesPlugin: PKPaymentAuthorizationViewControllerDelegate {
  ...
  public func paymentAuthorizationViewControllerDidFinish(
    ...
    flutterResult?(
      FlutterError(
        code: "APPLE_PAY_CANCELLED", message: "User cancelled Apple Pay",
        details: "Payment Cancelled"))
  }
  ...
  public func paymentAuthorizationViewController(
    ...
    guard let transactionDetails = self.storedRequestDetails else {
      ...
      flutterResult?(
        FlutterError(
          code: "MISSING_TRANSACTION_DETAILS", message: "Transaction details are missing",
          details: nil))
      return
    }
    ...
    if payment.token.paymentData.count != 0 {
      ...
      payFortController.callPayFortForApplePay(
        ...
        self.flutterResult?(
          FlutterError(code: "PAYMENT_FAILED", message: message, details: responseDic))
